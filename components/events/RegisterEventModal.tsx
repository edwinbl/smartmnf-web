...
export const RegisterEventModal = ({ open, onOpenChange, event }: Props) => {
  const isInvite = event?.status === "invite";
        ...
        >
          ...
          <DialogHeader className="space-y-2 text-left">
            <DialogTitle className="text-xl font-display font-bold text-white">
              {isInvite ? "Request your invitation" : "Reserve your spot"}
            </DialogTitle>
            <DialogDescription className="text-white/80 text-sm">
              {event
                ? `Create a free account to ${isInvite ? "request access to" : "register for"} "${event.title}", save events and unlock personalized recommendations.`
                : "Create a free account to register and save ecosystem events."}
            </DialogDescription>
          </DialogHeader>
        </div>
